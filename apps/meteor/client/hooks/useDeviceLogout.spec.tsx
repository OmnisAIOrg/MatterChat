import { mockAppRoot, type MockedAppRootBuilder } from '@rocket.chat/mock-providers';
...
type LogoutEndpointResponse = { sessionId: string };

/**
 * MATTERCHAT: `/v1/sessions/logout(.me)` are premium endpoints. Upstream declares them by
 * augmenting `Endpoints` from `apps/meteor/ee/server/api/sessions.ts`, a file this fork dropped
 * along with the rest of the EE tree, so neither path is a member of `PathPattern` and
 * `MockedAppRootBuilder.withEndpoint` rejects both. (`useDeviceLogout.tsx` hits the same gap on its
 * own `useEndpoint` call — that error predates the 8.7.0 merge.)
 *
 * Register the mock through a locally-typed view of the builder so the escape hatch stays in one
 * place: the response shape is still checked, and the hook under test is still called with the real
 * endpoint union. Remove this once the sessions endpoints are typed again.
 */
const withLogoutEndpoint = (builder: MockedAppRootBuilder, endpoint: Endpoint, response: () => LogoutEndpointResponse) =>
	(
		builder as unknown as {
			withEndpoint(method: 'POST', pathPattern: Endpoint, response: () => LogoutEndpointResponse): MockedAppRootBuilder;
		}
	).withEndpoint('POST', endpoint, response);
...
const setup = ({
	...
	let builder = withLogoutEndpoint(mockAppRoot().withQueryClient(queryClient), endpoint, logoutEndpoint)
		.withRouter({ navigate })
