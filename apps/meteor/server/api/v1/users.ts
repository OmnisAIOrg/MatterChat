...
// MATTERCHAT: self-serve firms — cohort scoping for user enumeration / users.info.
...
API.v1.get(
	...
	async function action() {
		...
		if (ids) {
			return API.v1.success({
				// MATTERCHAT: firm-cohort scoping wraps upstream's presence lookup (renamed from
				// findNotOfflineByIds to findPresenceUsersByIds in 8.7.0).
				users: scopeUsers(await Users.findPresenceUsersByIds(Array.isArray(ids) ? ids : ids.split(','), options).toArray()),
				full: false,
			});
		}
