 ...
 *
 * Both places that serve generated CSS/JS — server/lib/ui-master/inject.ts (the `css-theme`
 * and custom-script routes) and server/settings/theme/server.ts (`/theme.css`) — declared
 ...
 *
 *     /* MatterChat — OmnisAI house brand (GREEN) *␣/     510 chars, 512 bytes
 *
 ...
 * casualty is a render-blocking `<link rel="stylesheet">` in `<head>`, which means the entire
 * app hangs on a stylesheet that is never going to arrive. It presents as "the site won't
 * load", it only reproduces over HTTP/2 (i.e. through the load balancer, not against a local
 * dev server), and it is invisible to `curl` unless you force `--http2`.
 *
 * Two call sites, one subtle rule, and a failure mode that does not show up in development is
