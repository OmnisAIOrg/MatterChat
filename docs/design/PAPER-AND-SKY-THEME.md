...
| **Vendor prefix stripped** | These styles inject as runtime `<style>` tags, which **bypass PostCSS/autoprefixer entirely**. `-webkit-backdrop-filter` must be hand-written beside every `backdrop-filter` or Safari and the Electron shell lose all glass. The canonical stylesheet carries the same warning. |
