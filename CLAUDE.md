...
- **NEVER blanket `eslint --fix` a directory and `git add -A` unreviewed** — the import-order autofix cannot resolve `meteor/*` packages and once MERGED `import { RoutePolicy } from 'meteor/routepolicy'` into the `meteor/ostrio:cookies` import → `RoutePolicy` undefined → server crash-loop at boot (2026-07-20, compiled clean; `meteor/*` is ambient to tsc). Autofix ONLY the files you touched, then review the import lines of the diff before committing.
- **sendMessage() has silent-by-default validation gates** that swallow programmatic messages into warn-only logs: `alias`/`avatar` ⇒ requires `message-impersonate` (roles `bot`/`app` only — send aliased bridge/import messages as a BOT), and `customFields` ⇒ rejected unless the workspace `Message_CustomFields` setting is on (stamp internal metadata AFTER save via `Messages.updateOne`, like the bridge outbound leg). Cost DAYS as "flaky Slack inbound". First debugging move for any eaten-message pipeline: grep the server log — the exact error is almost always already there.
- **First model/HTTP call right after `SERVER RUNNING` can fail** with node-fetch `reason: (empty)` (races Meteor boot) — wait ~8s before probing; not a code bug.
- **Long-lived feature branches conflict with their own squash-merges**: PRs squash onto `staging`, so a branch stacked on the ORIGINAL commits goes `CONFLICTING` once its ancestor PR lands. Resolve by merging `origin/staging` in and taking the branch side for files the branch strictly supersedes.
- **CodeQL-Build fails repo-wide** (infra flake on the monorepo, predates 2026-07-20) and is NOT a required check — a PR blocked ONLY by CodeQL is mergeable; `mergeStateStatus` shows `UNSTABLE`, not `CLEAN`. Gate automation on `mergeable == MERGEABLE`, then verify state == MERGED after.
